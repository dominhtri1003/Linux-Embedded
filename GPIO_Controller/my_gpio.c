...
static struct my_gpio_bank *to_my_gpio_bank(struct gpio_chip *chip)
{
    ...
    if(ret == NULL)
        pr_err("Can not get bank my chip");
    return ret;
}
...
int __get_direction(struct my_gpio_bank *bank, int pin)
{
    ...
    temp_reg &= (1 << pin); /* Check bit corresponding to pin */
...
{
    void __iomem *base = bank->base;
    int temp_reg = ioread32(base + bank->regs->datain);
...
void __set_irq_mode(struct my_gpio_bank *bank, int pin, char detection)
{
    ...
    switch(detection){
        ...
        default:
            ...
            "Could not config interrupt for pin %d\n", pin);
            break;
    }
...
int my_gpio_direction_output(struct gpio_chip *chip, unsigned int pin, int value)
{
    struct my_gpio_bank *bank = to_my_gpio_bank(chip);
...
int my_gpio_set_config(struct gpio_chip *chip, unsigned int pin, unsigned long config)
{
    struct my_gpio_bank *bank = to_my_gpio_bank(chip);
...
static int init_gpio_chip(struct platform_device *pdev, struct irq_chip *irqc)
{
    ...
    if (bank == NULL)
        dev_err(dev, "can not get drvdata\n");
    ...
    if (ret)
    {
        dev_err(bank->chip.parent, "TriDo Could not register gpio chip\n");
		return ret;
    }
    ...
    /* request irq */
    ret = devm_request_irq(bank->chip.parent, bank->irq, my_irq_handler, 0, dev_name(bank->chip.parent), bank);
    if (ret)
    {
        pr_emerg("TriDo %s %d\n", __func__, __LINE__);
		gpiochip_remove(&bank->chip);
...
static int my_gpio_probe(struct platform_device *pdev)
{
    ...
    irqc = devm_kzalloc(dev, sizeof(struct my_gpio_bank), GFP_KERNEL);
    if (!bank) {
		pr_err("Error allocate memmory\n");
		return -ENOMEM;
	}
